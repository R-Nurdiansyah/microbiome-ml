...
def low_quality_metadata_data():
    ...
    return {
        ...
        "biosample": ["BS_LQ1", "BS_LQ2", "BS_LQ3", "BS_DOM1", "BS_DOM2", "BS_HQ1", "BS_HQ2"],
        "bioproject": ["BP1", "BP1", "BP2", "BP2", "BP2", "BP3", "BP3"],
        "lat": [15.0, 25.0, 35.0, 45.0, 50.0, 55.0, 60.0],
        "lon": [45.0, 55.0, 65.0, 75.0, 80.0, 85.0, 90.0],
        "collection_date": ["2020-05-01", "2020-06-01", "2020-07-01", "2020-08-01", "2020-08-15", "2020-09-01", "2020-09-15"],
        "biome": ["soil", "marine", "soil", "marine", "marine", "freshwater", "freshwater"],
        "mbases": [500, 800, 950, 1500, 1600, 2000, 2200]  # First 3 below 1000, DOM1/DOM2/HQ1/HQ2 above
    }
...
def low_quality_attributes_csv(tmp_path):
    ...
    data = {
        "sample": ["LQ1", "LQ1", "LQ2", "LQ2", "LQ3", "LQ3", "DOM1", "DOM1", "DOM2", "DOM2", "HQ1", "HQ1", "HQ2", "HQ2"],
        "key": ["pH", "temp"] * 7,
        "value": ["6.0", "20", "7.0", "21", "6.5", "22", "7.3", "25", "7.4", "26", "7.2", "23", "6.8", "24"]
    }
...
def low_coverage_profiles_data():
    ...
    return {
        ...
        "taxonomy": [
            "d__Bacteria", "d__Bacteria;p__Proteobacteria",
            "d__Bacteria", "d__Bacteria;p__Firmicutes",
            "d__Bacteria", "d__Bacteria;p__Actinobacteria",
            "d__Bacteria", "d__Bacteria;p__Bacteroidetes"
        ] * 3,
...
def low_coverage_root_data():
    ...
    return {
        ...
        "root_coverage": [30.0, 45.0, 100.0, 150.0]  # First 2 below 50 threshold
    }
...
def large_metadata_data():
    ...
    return {
        ...
        "bioproject": [f"BP{i % 5}" for i in range(n_samples)],  # 5 different projects
        "lat": np.random.uniform(-90, 90, n_samples).tolist(),
        "lon": np.random.uniform(-180, 180, n_samples).tolist(),
        "collection_date": [f"2020-{(i % 12) + 1:02d}-{(i % 28) + 1:02d}" for i in range(n_samples)],
        "biome": np.random.choice(["soil", "marine", "freshwater", "sediment"], n_samples).tolist(),
        # Create varying quality: 30% below 1000, 70% above
        "mbases": (np.random.choice([500, 700, 900], size=30).tolist() + 
...
def large_profiles_data():
    ...
    for i in range(n_samples):
        ...
        if i < 20:
            # Dominated samples
            abundances = [1.0, 0.995, 0.99, 0.99, 0.005, 0.003, 0.002, 0.0, 0.0, 0.0]
        else:
