...
Feature-importance plotting accepts rich result objects directly:
- `plot_feature_importances(cv_result)` for a single CV result
- `plot_feature_importances(holdout_evaluation)` for the final holdout model
- `plot_feature_importances(model_path)` for a saved pickle model
...
Usage:
    ...
    # New: directly from result containers (no manual feature-name wiring)
    vis.plot_feature_importances(cv.best_result, output="cv_importance.png")
    vis.plot_feature_importances(
        evaluation,
        output="holdout_importance.png",
    )
"""
...
class Visualiser:
    ...
    def plot_cv_bars(
        ...
        - Each group's file is saved as <feature_set>__<label>__<scheme>__<model>.png in `out_dir` (defaults to `self.output_dir / "cv_results"`).
        """
    ...
    def plot_feature_importances(
        ...
        Accepted inputs for `model`:
        - `CV_Result`: uses `result.model`, `result.feature_names`, and
            feature-set metadata when available.
        - `HoldoutEvaluation`: uses `evaluation.estimator`,
            `evaluation.feature_names`, and `evaluation.metrics` metadata.
        - fitted estimator object exposing `feature_importances_`.
        - dict payload containing `{"model": estimator}`.
        - pickle path (str/Path) to a persisted estimator.

        Notes:
        - If `feature_names` is not provided, names are inferred from result
            metadata, then estimator `feature_names_in_`, then autogenerated as
            `feature_0..feature_n`.
        - When lengths mismatch, the shorter length is used.

        Example:
                vis.plot_feature_importances(cv.best_result, output="cv_fi.png")
                holdout = trainer.train_and_evaluate()
                vis.plot_feature_importances(holdout, output="holdout_fi.png")
        """
        feature_set = None
        label = None
        scheme_from_path = None
        model_obj = None

        if isinstance(model, CV_Result):
            model_obj = model.model
            feature_set = model.feature_set
            label = model.label
            scheme_from_path = model.scheme
            if feature_names is None:
                feature_names = list(model.feature_names)
        elif isinstance(model, HoldoutEvaluation):
            ...
            scheme_from_path = metrics.get("scheme")
            if feature_names is None and model.feature_names is not None:
                feature_names = list(model.feature_names)

        if isinstance(model, (str, Path)):
            model_path = Path(model)
            if not model_path.exists():
                logging.warning("Pickle model path not found: %s", model_path)
                return
            try:
                import pickle
                ...
                # Attempt to infer feature_set/label/scheme from folder name
                parent_name = model_path.parent.name
                if ";" in parent_name:
                    parts = parent_name.split(";")
                    if len(parts) >= 3:
                        feature_set, label, scheme_from_path = parts[:3]
            except Exception as exc:
                logging.warning("Failed to load pickled model: %s", exc)
                return
        ...
        if not hasattr(model_obj, "feature_importances_"):
            logging.warning(
                "Model does not expose feature_importances_. Skipping plot."
            )
            return

        importances = np.asarray(model_obj.feature_importances_)
        if importances.size == 0:
            logging.warning("Feature importances are empty. Skipping plot.")
            return
        if feature_names is None:
            inferred = getattr(model_obj, "feature_names_in_", None)
            if inferred is not None:
                feature_names = [str(name) for name in list(inferred)]
            else:
                feature_names = [
                    f"feature_{i}" for i in range(importances.size)
                ]
        if len(feature_names) != importances.size:
            logging.warning(
                "Feature names length (%s) does not match importances (%s).",
                len(feature_names),
                importances.size,
            )
            n = min(len(feature_names), importances.size)
            if n == 0:
                logging.warning(
                    "No aligned feature names found. Skipping plot."
                )
                return
        ...
        if title == "Feature Importances":
            title_parts: List[str] = []
            if model_name:
                title_parts.append(model_name)
            elif model_obj is not None:
                title_parts.append(model_obj.__class__.__name__)
            if feature_set or label or scheme_from_path or scheme:
                fs = feature_set or "feature_set"
                lb = label or "label"
                sc = scheme or scheme_from_path or "scheme"
                title_parts.append(f"{fs} - {lb} - {sc}")
            if title_parts:
                title = " Feature Importances | ".join(
