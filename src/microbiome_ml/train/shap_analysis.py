...
class SHAPResult:
    ...
    Attributes:
        ...
        mean_abs_shap: Per-feature ``mean(|SHAP|)``, computed on init.
    """
    ...
    def save(self, path: Union[str, Path]) -> None:
        ...
        logger.info("Saved SHAP values (%d rows) to %s",
                    len(self.shap_values), outp)

    def save_summary(self, path: Union[str, Path]) -> None:
        """Write mean |SHAP| summary ranked by importance."""
        outp = Path(path)
        outp.parent.mkdir(parents=True, exist_ok=True)
        order = np.argsort(self.mean_abs_shap)[::-1]
        with outp.open("w", encoding="utf-8", newline="") as fh:
            ...
            for rank, idx in enumerate(order, start=1):
                writer.writerow(
                    [
                        ...
                        float(self.mean_abs_shap[int(idx)]),
                    ]
                )
        logger.info("Saved SHAP summary (%d features) to %s",
                    len(self.feature_names), outp)
...
class SHAPAnalyser:
    ...
    def compute(self) -> SHAPResult:
        ...
        if self._is_tree_model(self.model):
            logger.info("TreeExplainer for %s (background=%d)",
                        model_name, len(background))
            ...
            logger.info("Generic shap.Explainer for %s (background=%d)",
                        model_name, len(background))
